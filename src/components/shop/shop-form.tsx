...
import { CREATE_SHOP, SHOP_LIST, UPDATE_SHOP } from '@/config/endPoints';
...
const ShopForm = ({ initialValues }: { initialValues?: any}) => {
  ...
  const [shopData,setShopData] = useState(JSON.parse(localStorage.getItem('dataForUpdate')));
 ...
 async function onSubmit(values: FormValues) {
  ...
  const getShops=async(shopId:any)=>{
    // let shopdata = await httpGet(`${SHOP_LIST}/${shopId}`);
    // console.log('----shopdata',shopdata)
    // if(shopdata.status == 200){
    //   setShopData(shopdata.data.users);
    //   localStorage.setItem('dataForUpdate',JSON.stringify(shopdata.data.users))
    // }
    let shopdata = await httpGet(`${SHOP_LIST}`);
    if(shopdata.data.users.length > 0){
      setShopData(shopdata.data.users[0]);
      localStorage.setItem('dataForUpdate',JSON.stringify(shopdata.data.users[0]))
    }else{
      setShopData()
    }
  }
  ...
  useEffect(()=>{
    let shopId = localStorage.getItem('idForUpdate')
    console.log('---data----00',getShops(shopId))
  },[])
