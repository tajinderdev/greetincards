...
console.log('---------------',userProfile)
  ...
  return (
    <div className="flex h-full flex-col items-center p-5">
      ...
      <h3 className="mt-4 text-lg font-semibold text-heading">{userProfile?.name}</h3>
      <p className="mt-1 text-sm text-muted">{userProfile?.email}kkk</p>
      {!profile ? (
        ...
        <>
          <p className="mt-0.5 text-sm text-muted">{userProfile?.contact}</p>
        </>
      )}
