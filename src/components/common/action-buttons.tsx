...
const ActionButtons = ({
  ...
  function handleDelete() {
    openModal(deleteModalView, id,keyName);
  }
  ...
  const updateIdForShop=()=>{
    localStorage.setItem('idForUpdate',id)
    localStorage.setItem('dataForUpdate',JSON.stringify(data))
    console.log('hhhhhhhh',name)
  }
    ...
    <div className="gap-8 inline-flex w-auto items-center">
      ...
      {detailsUrl && (
        <Link
          ...
          locale={customLocale}
        >
         <span onClick={updateIdForShop}> <Eye width={24} /></span>
        </Link>
      )}
    </div>
  );
